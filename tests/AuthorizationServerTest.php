...
use Drewlabs\Txn\TMoney\AuthorizationConfigServer;
...
class AuthorizationServerTest extends TestCase
{
    ...
    {
        $server = new AuthorizationConfigServer('http://127.0.0.1:3000', '/api/oauth/token');

        $this->assertInstanceOf(AuthorizationServerConfigInterface::class, $server);
    }
    ...
    {
        $server = new AuthorizationConfigServer('http://127.0.0.1:3000', 'http://auth.example.com/api/oauth/token');
